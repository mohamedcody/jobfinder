...
export default function LoginPage() {
  ...
  const [lockUntil, setLockUntil] = useState<number | null>(() => {
    ...
    if (!Number.isNaN(parsed) && parsed > getNowMs()) return parsed;
  ...
  return (
    <AuthShell
      title="Welcome back"
      description="Sign in to access your career dashboard."
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" noValidate>
          ...
          <SubmitButton isLoading={isSubmitting} disabled={lockSecondsRemaining > 0}>
            {lockSecondsRemaining > 0 ? `Too many attempts. Try again in ${lockSecondsRemaining}s` : "Sign In"}
          </SubmitButton>
        </form>
