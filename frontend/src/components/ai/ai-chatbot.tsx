...
type TriggerAiChatDetail = {
  text?: string;
  autoSend?: boolean;
};

const isTriggerAiChatEvent = (event: Event): event is CustomEvent<TriggerAiChatDetail> => "detail" in event;
...
export default function AiChatbot() {
  ...
  useEffect(() => {
    const handleTriggerChat = (e: Event) => {
      if (!isTriggerAiChatEvent(e)) return;
      const detailText = typeof e.detail?.text === "string" ? e.detail.text : "";
      const shouldAutoSend = Boolean(e.detail?.autoSend);
      setIsOpen(true);
      if (shouldAutoSend) {
        sendMessage(detailText);
      } else {
        setInput(detailText);
      }
    };
